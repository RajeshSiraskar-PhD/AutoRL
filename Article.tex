...
We model the maintenance decision problem as an episodic Markov decision process (MDP) $\mathcal{M}=(\mathcal{S},\mathcal{A},P,r,\gamma)$~\cite{SuttonBarto2018}. At discrete timestep $t$, the agent observes a sensor vector $\mathbf{x}_t\in\mathbb{R}^d$ and chooses an action
\begin{equation}
...
\begin{equation}
  s_t \equiv \mathbf{x}_t = \mathrm{Sensors}(t),\qquad w_t\notin s_t.
\end{equation}
...
Attention can be written as a mapping from queries $Q$, keys $K$, and values $V$ to a context representation
\begin{equation}
  \mathrm{Att}(Q,K,V) = \mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V,
\end{equation}
popularized by Transformer architectures~\cite{Vaswani2017Attention}. In maintenance, attention is attractive because it can (i) emphasize salient sensors, (ii) highlight interactions among signals, and (iii) provide a structured inductive bias without necessarily increasing end-to-end model complexity dramatically.

In this work, attention is implemented as a \emph{feature extractor} $\phi_\theta(\mathbf{x}_t)$ inside the policy/value networks (Stable-Baselines3 custom extractors). This shifts attention from ``sequence-to-sequence'' NLP settings to ``feature weighting/fusion'' over multi-sensor inputs.
...
\paragraph{Theory.}
Self-attention often improves modeling of feature interactions. In Transformers it operates over tokens; here it operates over a projected feature vector.
...
\paragraph{Pros/cons.}
This design can act as a batchwise gating mechanism and may stabilize training via normalization and residual structure. However, batchwise normalization is not equivalent to standard token-wise attention and may introduce sensitivity to batch composition.
